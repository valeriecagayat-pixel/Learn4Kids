...
function initBackButton() {
  ...
  if (backBtn) {
    backBtn.addEventListener('click', function(event) {
      // Prevent the browser from jumping to '#' top of page
      event.preventDefault(); 
      
      // Safely navigate to the previous page in the user's browser history
      if (window.history.length > 1) {
        window.history.back();
      } else {
        // Fallback: If there is no previous history, take them back to the home page safely
...
// Run the initialization safely once the DOM content loads
