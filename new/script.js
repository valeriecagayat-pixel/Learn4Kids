...
function drawGuide(canvas, ctx) {
  ...
  ctx.font = "bold 60px Arial"; // UNIFORM FONT SIZE
...
function checkHenyoAnswer(buttonElement, selectedChoice) {
  speakWord(selectedChoice);

  const card = buttonElement.closest('.skill-card');
  if (!card) return;

  const categorySpan = card.querySelector('.category');
  if (!categorySpan) return;
  
  let categoryKey = categorySpan.innerText.trim();
  
  if (categoryKey === "Animals") {
    const clueText = card.querySelector('p').innerText.toLowerCase();
    if (clueText.includes("sky")) {
      categoryKey = "Animals-Sky";
    } else if (clueText.includes("jump") || clueText.includes("ribbit")) {
      categoryKey = "Animals-Jump";
    }
  }

  const correctAnswer = henyoAnswers[categoryKey];

  let feedback = card.querySelector('.henyo-feedback');
  if (!feedback) {
    feedback = document.createElement('p');
    feedback.className = 'henyo-feedback';
  ...
  if (selectedChoice === correctAnswer) {
    feedback.innerText = `🎉 Correct! It is an awesome ${selectedChoice}! 🌟`;
    feedback.style.color = "#1DD1A1";
  } else {
    feedback.innerText = `❌ Not quite right! You selected ${selectedChoice}. Try again!`;
    feedback.style.color = "#FF6B6B";
  }
}